...
void setup_socket(void)
{
	...
	getaddrinfo("0.0.0.0", ARTNET_PORT, &hints, &res);
...
void wait_replies(void)
{
	...
	while (1) {
		...
		if (pkt->OpCode != OpPollReply) {
			printf("Received packet with unexpected opcode %04x\n",pkt->OpCode);
			continue;
		}
		printf("Found device on %d.%d.%d.%d: %s\n", pkt->IPAddress[0],
			pkt->IPAddress[1], pkt->IPAddress[2], pkt->IPAddress[3],
			pkt->ShortName);
		printf("  Manufacturer: %02x%02x: Firmware version 0x%04x\n",
			pkt->EstaManHi, pkt->EstaManLo,
